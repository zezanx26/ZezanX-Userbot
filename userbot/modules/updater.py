    ...
    if HEROKU_API_KEY is not None:
        ...
        if HEROKU_APP_NAME is None:
            await edit_or_reply(event,
                                ...
                                " untuk dapat deploy perubahan terbaru dari ✨ᴢᴇᴢᴀɴ-ᴜsᴇʀʙᴏᴛ✨.`"
                                )
            repo.__del__()
            return
        ...
        if build.status == "failed":
            ...
            await edit_delete(event,
                              "`ZezanX-Userbot Berhasil DiUpdate🛃,Restart Tunggu Sebentar`"
                              )

        if BOTLOG:
            await event.client.send_message(
                BOTLOG_CHATID, "#BOT \n" "`ZezanX-Userbot Berhasil Di Update`"
            )
...
async def update(event, repo, ups_rem, ac_br):
    ...
    x = await edit_or_reply(event, "**✨ᴢᴇᴢᴀɴ-ᴜsᴇʀʙᴏᴛ✨** `Berhasil Di Update!`")
    await asyncio.sleep(1)
    await x.edit("**✨ᴢᴇᴢᴀɴ-ᴜsᴇʀʙᴏᴛ✨** `Di Restart....`")
    await asyncio.sleep(1)
    ...
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID, "#BOT \n" "**✨ᴢᴇᴢᴀɴ-ᴜsᴇʀʙᴏᴛ✨ Telah Di Perbarui.**"
        )
...
async def upstream(event):
    ...
    if changelog == "" and force_update is False:
        await xx.edit(
            f"\n✨ᴢᴇᴢᴀɴ-ᴜsᴇʀʙᴏᴛ✨ Sudah Versi Terbaru || Tunggu Update Terbaru\n"
        )
    ...
    if conf is None and force_update is False:
        changelog_str = (
            f"**Pembaruan Untuk ✨ᴢᴇᴢᴀɴ-ᴜsᴇʀʙᴏᴛ✨ :\n\n⚒️ Pembaruan Data :**\n`{changelog}`"
        )
    ...
    if force_update:
        ...
        await xx.edit("` Proses Update ✨ᴢᴇᴢᴀɴ-ᴜsᴇʀʙᴏᴛ✨, Loading....1%`")
        await xx.edit("` Proses Update ✨ᴢᴇᴢᴀɴ-ᴜsᴇʀʙᴏᴛ✨ Loading....20%`")
        await xx.edit("` Proses Update ✨ᴢᴇᴢᴀɴ-ᴜsᴇʀʙᴏᴛ✨, Loading....35%`")
        await xx.edit("` Proses Update ✨ᴢᴇᴢᴀɴ-ᴜsᴇʀʙᴏᴛ✨, Loading....77%`")
        await xx.edit("` Proses Update ✨ᴢᴇᴢᴀɴ-ᴜsᴇʀʙᴏᴛ✨, Updating...90%`")
        await xx.edit(
            "` Proses Update ✨ᴢᴇᴢᴀɴ-ᴜsᴇʀʙᴏᴛ✨, Mohon Tunggu Sebentar....100%`"
        )
...
CMD_HELP.update(
    {
        ...
        "\n• : Untuk Melihat Pembaruan Terbaru ZezanX-Userbot."
        f"\n\n𝘾𝙤𝙢𝙢𝙖𝙣𝙙: `{cmd}update now`"
        "\n• : Memperbarui ZezanX-Userbot."
        f"\n\n𝘾𝙤𝙢𝙢𝙖𝙣𝙙: `{cmd}update deploy`"
        "\n• : Memperbarui ZezanX-Userbot Dengan Cara Men-Deploy Ulang."
    }
)
